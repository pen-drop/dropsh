...
describe("oauth2-authcode adapter", () => {
  it("attaches Bearer header when access token is still valid", async () => {
    await withTmpDir(async (dir) => {
      ...
      const adapter = createOAuth2AuthCodeAuth(CFG, { http: httpMock([]), baseUrl: BASE, now: () => 0, tokenDir: dir });
  ...
  it("silently refreshes when access token is expired", async () => {
    await withTmpDir(async (dir) => {
      const http = httpMock([{ status: 200, body: { access_token: "new-tok", refresh_token: "new-ref", expires_in: 3600 } }]);
      await writeToken(BASE, { access_token: "old-tok", refresh_token: "old-ref", expires_at: 1000 }, dir);
      const adapter = createOAuth2AuthCodeAuth(CFG, { http, baseUrl: BASE, now: () => 2_000_000, tokenDir: dir });
      const req = await adapter.apply({ method: "GET", url: "https://example.com/jsonapi" });
  ...
  it("uses old refresh_token when server does not return a new one", async () => {
    await withTmpDir(async (dir) => {
      ...
      await writeToken(BASE, { access_token: "old-tok", refresh_token: "keep-ref", expires_at: 1000 }, dir);
      const adapter = createOAuth2AuthCodeAuth(CFG, { http, baseUrl: BASE, now: () => 2_000_000, tokenDir: dir });
      await adapter.apply({ method: "GET", url: "https://example.com" });
      const stored = await readToken(BASE, dir);
  ...
  it("throws AuthError when access token expired and no refresh_token stored", async () => {
    await withTmpDir(async (dir) => {
      ...
      const adapter = createOAuth2AuthCodeAuth(CFG, { http: httpMock([]), baseUrl: BASE, now: () => 2_000_000, tokenDir: dir });
      await expect(adapter.apply({ method: "GET", url: "https://example.com" })).rejects.toBeInstanceOf(AuthError);
    });
  });
  ...
  it("throws AuthError when no token file exists", async () => {
    await withTmpDir(async (dir) => {
      const adapter = createOAuth2AuthCodeAuth(CFG, { http: httpMock([]), baseUrl: BASE, tokenDir: dir });
      await expect(adapter.apply({ method: "GET", url: "https://example.com" })).rejects.toBeInstanceOf(AuthError);
    });
  });
