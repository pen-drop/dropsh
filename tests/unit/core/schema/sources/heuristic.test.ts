...
describe("fetchHeuristic", () => {
  it("builds shallow schema from 3 sample records", async () => {
    const body = readFileSync(
      resolve(__dirname, "../../../fixtures/samples/node--article-3-records.json"),
      "utf8",
    );
    const { schema, empty } = await fetchHeuristic({
      http: http(body),
      auth,
      baseUrl: "https://ex",
  ...
  it("returns empty-flag true for a bundle with no instances", async () => {
    const empty = readFileSync(
      resolve(__dirname, "../../../fixtures/samples/node--empty-bundle.json"),
      "utf8",
    );
    const result = await fetchHeuristic({
      http: http(empty),
      auth,
      baseUrl: "https://ex",
      jsonapiPrefix: "/jsonapi",
