...
// constraint mechanism (required field + maxLength) on the fixture's
// article_test bundle, which the heuristic schema could never enforce.

describe("integration: jsonapi-schema plugin", () => {
  it("serves an authoritative, constraint-bearing schema for create", async () => {
    // Seed one instance so the heuristic base resolves the bundle before the
    // plugin replaces it (mirrors the schema-refresh integration test).
    await createTestNode(`js-${crypto.randomUUID()}`, "jsonapischema");
    const res = await runCli({
      site: "jsonapischema",
      args: ["schema", "node/article_test", "--for", "create"],
    });
    ...
    // The node title is required, with Drupal's 255-char limit.
  ...
  it("serves an authoritative schema for update", async () => {
    const res = await runCli({
      site: "jsonapischema",
      args: ["schema", "node/article_test", "--for", "update"],
    });
    expect(res.code).toBe(0);
    const schema = parseJson<Record<string, unknown>>(res.stdout);
    expect(schema["x-dropsh-source"]).toBe("jsonapi-schema");
  ...
  it("rejects an invalid payload client-side without --no-validate", async () => {
    ...
    // is in play.
    const payload = { data: { type: "node--article_test", attributes: {} } };
    const res = await runCli({
      site: "jsonapischema",
  ...
  it("accepts the same payload when the required field is present", async () => {
    const payload = {
      data: { type: "node--article_test", attributes: { title: `js-${crypto.randomUUID()}` } },
    };
    const res = await runCli({
      site: "jsonapischema",
      args: ["create", "node", "--bundle=article_test", `--data=${JSON.stringify(payload)}`],
